...
Public Class inputsForm
    ...
    Public Sub New(NetworkID As String, SKU As String, NewNetwork As Boolean, NewSku As Boolean)
        ...
        ''Read only strings for Config
        Me.googleAPIKey = "AIzaSyAMUKAXQGyMugW1bvMPpsaq4wyk-v5fWiQ"
        Me.client = New RestClient("https://maps.googleapis.com/")
        Me.connectionString = "Server=180.150.46.156;Database=prophit_FM_DEMO;User Id=Prophit;Password=foodcan;Encrypt=False"



        ''All tabs are first disabled
        ''WarehouseLocationsTab.Enabled = False
        'ItemInputsTab.Enabled = False
        'WarehouseInputsTab.Enabled = False
        'WarehouseRelationsTab.Enabled = False
    ...
    ''' updates the database or inserts depends if the warehouse already exists
    ''' </summary>
    ...
    ''' <summary>
    ''' THis function Takes the global Reorder Relationship Values and adds them to the database. It does an UPSERT
    ''' and updates the database or inserts depending if the reorder relationship already exists
    ''' </summary>
    Private Sub updateDatabaseReorderRelationship()
        Dim query As String = "
            MERGE INTO prophit.[OPT.Reorder_Relation] AS Target
            USING (VALUES (@Network_ID, @Warehouse_ID, @Warehouse_from_ID, @Lead_time_mean, @Lead_time_std, @Reorder_cost)) AS source (Network_ID, Warehouse_ID, Warehouse_from_ID, Lead_time_mean, lead_time_std, Reorder_cost)
            ON Target.Network_ID = source.Network_ID AND Target.Warehouse_ID = source.Warehouse_ID AND Target.Warehouse_from_ID = source.Warehouse_from_ID
            WHEN MATCHED THEN
                UPDATE SET Lead_time_mean = source.Lead_time_mean, Lead_time_std = source.Lead_time_std, Reorder_cost = source.Reorder_cost
            WHEN NOT MATCHED THEN
                INSERT (Network_ID, Warehouse_ID, Warehouse_from_ID, Lead_time_mean, Lead_time_std, Reorder_cost)
                VALUES (source.Network_ID, source.Warehouse_ID, source.Warehouse_from_ID, source.Lead_time_mean, source.Lead_time_std, source.Reorder_cost);"
            ...
            Try
                ...
                For Each relation In Me.reorderInputs
                    ...
                    Using cmd As New SqlCommand(query, conn)
                        cmd.Parameters.AddWithValue("@Network_ID", Me.NetworkID)
                        cmd.Parameters.AddWithValue("@Warehouse_ID", warehouseID)
                        cmd.Parameters.AddWithValue("@Warehouse_from_ID", reorderFromID)
                        cmd.Parameters.AddWithValue("@Lead_time_mean", reorderInfo.lead_time_mean)
                        cmd.Parameters.AddWithValue("@Lead_time_std", reorderInfo.lead_time_sd)
                        cmd.Parameters.AddWithValue("@Reorder_cost", reorderInfo.reorder_cost)
                        cmd.ExecuteNonQuery()
                    End Using
                Next
            Catch ex As Exception
    ...
    Private Sub WarehouseAndSKUInputsLoadtab()
        ...
        If WarehouseAndSkuInputs IsNot Nothing Then
            For Each warehouseInput In WarehouseAndSkuInputs
                If WarehouseStillExists(warehouseInput.warehouse_id) Then
    ...
    Private Function WarehouseInTable(WarehouseID As Integer, table As DataGridView)
        For Each row In table.Rows
            If Not row.IsNewRow AndAlso row.Cells(0).Value IsNot Nothing Then
                If row.Cells(0).Value.ToString() = WarehouseID Then
                    Return True
                End If
            End If
        Next
    ...
    Private Sub btnSubmitWarehouseSkuData_Click(Sender As Object, e As EventArgs) Handles btnNextWarehouse.Click
        ...
        If validateWarehouseInputs() Then
            ...
            MessageBox.Show("Warehouse and SKU inputs have been successfully submitted")
